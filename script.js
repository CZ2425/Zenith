...
const promptHandlers = [
  {
    name: "math",
    patterns: [
      /\b(calculate|compute|solve|math|evaluate)\b/i,
      /\bwhat\s+(is|are)\b.*(\d|pi|π|sqrt|square root)/i,
      /(\d|pi|π|e)\s*(plus|minus|times|multiplied by|divided by|over|percent of|squared|cubed|[+\-*/^%=])/i,
...
function solveMathPrompt(prompt) {
  ...
  const displayExpression = formatMathExpression(expression);
  ...
  if (expression.includes("=")) {
    ...
    if (solution !== null) {
      return `Math result: solving ${displayExpression} gives x = ${formatMathResult(solution)}.`;
    }

    return `I found the equation “${displayExpression},” but this local math engine currently solves one-variable linear equations only.`;
  }

  const result = evaluateMathExpression(expression);

  if (result === null) {
    return `I found “${displayExpression},” but I could not calculate it. Try arithmetic with +, -, *, /, ^, %, parentheses, sqrt(), or a simple equation with x.`;
  }

  return `Math result: ${displayExpression} = ${formatMathResult(result)}.`;
}
